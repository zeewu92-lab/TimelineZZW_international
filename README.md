...
目前 `public/` 裡的圖示（`icon-192.png`、`icon-512.png`、`icon-maskable-512.png`、`apple-touch-icon.png`、`favicon.ico`）是暫時生成的簡易佔位圖，建議之後換成正式設計的品牌圖示，直接覆蓋同名檔案即可，不用改程式碼。

若之後想升級成能上架 App Store / Google Play 的原生 App，可以在這個基礎上改用 [Capacitor](https://capacitorjs.com/) 包裝，現有的 React 程式碼幾乎不用大改。

### 中國大陸可用性

- **靜態頁面本身**：不建議長期依賴 `xxx.vercel.app` 這種預設網域，大陸網路環境對它的解析／連線不穩定。建議改綁自訂網域並完成 ICP 備案，部署到阿里雲 OSS+CDN、騰訊雲 COS+CDN 等大陸可直接訪問的靜態託管。
...
To enable login and cloud sync, create a project in the [Firebase Console](https://console.firebase.google.com/) and fill in the corresponding values in `firebaseConfig` inside `src/lib/firebase.js` (a working development-only config is currently included).
...
The icons currently in `public/` (`icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`, `favicon.ico`) are temporary placeholder graphics. Feel free to replace them with your final branded icons — just overwrite the files with the same names, no code changes needed.

If you later want to ship this as a native app on the App Store / Google Play, you can wrap it with [Capacitor](https://capacitorjs.com/) on top of this codebase, with little change needed to the existing React code.

### Availability in Mainland China

- **The static site itself**: relying long-term on a default domain like `xxx.vercel.app` isn't recommended, since DNS resolution and connectivity to it can be unstable from within mainland China. Consider using a custom domain with ICP filing, deployed on a mainland-accessible static host such as Alibaba Cloud OSS+CDN or Tencent Cloud COS+CDN.
...
[Node.js](https://nodejs.org/)（推奨バージョン 18 以上）が必要です。
...
ログインとクラウド同期機能を有効にするには、[Firebase Console](https://console.firebase.google.com/) でプロジェクトを作成し、`src/lib/firebase.js` 内の `firebaseConfig` に対応する値を入力してください（現在のファイルには開発テスト用の設定が含まれています）。
...
現在 `public/` 内のアイコン（`icon-192.png`、`icon-512.png`、`icon-maskable-512.png`、`apple-touch-icon.png`、`favicon.ico`）は仮の簡易プレースホルダー画像です。正式なブランドアイコンに差し替える際は、同名のファイルを上書きするだけでよく、コードの変更は不要です。

将来的に App Store / Google Play でネイティブアプリとして公開したい場合は、このコードベースの上に [Capacitor](https://capacitorjs.com/) でラップすることができ、既存の React コードはほとんど変更する必要がありません。

### 中国本土での利用について

- **静的サイト自体**：`xxx.vercel.app` のようなデフォルトドメインに長期的に依存することは推奨されません。中国本土のネットワーク環境では名前解決や接続が不安定になることがあります。カスタムドメインを使用し ICP 備案を完了した上で、Alibaba Cloud OSS+CDN や Tencent Cloud COS+CDN など中国本土から直接アクセス可能な静的ホスティングへのデプロイを推奨します。
...
[Node.js](https://nodejs.org/)（18 이상 권장）가 설치되어 있어야 합니다.
...
로그인 및 클라우드 동기화 기능을 사용하려면 [Firebase Console](https://console.firebase.google.com/)에서 프로젝트를 생성하고, `src/lib/firebase.js`의 `firebaseConfig`에 해당 값을 입력해야 합니다 (현재 파일에는 개발/테스트용 설정이 포함되어 있습니다).
...
현재 `public/`에 있는 아이콘 (`icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`, `favicon.ico`)은 임시로 생성된 플레이스홀더 이미지입니다. 정식 브랜드 아이콘으로 교체할 때는 동일한 이름의 파일을 덮어쓰기만 하면 되며, 코드 수정은 필요하지 않습니다.

추후 App Store / Google Play에 출시할 네이티브 앱으로 업그레이드하고 싶다면, 이 코드베이스를 기반으로 [Capacitor](https://capacitorjs.com/)로 감싸는 방식을 사용할 수 있으며, 기존 React 코드는 거의 수정할 필요가 없습니다.

### 중국 본토 이용 가능성

- **정적 페이지 자체**: `xxx.vercel.app`와 같은 기본 도메인에 장기적으로 의존하는 것은 권장하지 않습니다. 중국 본토 네트워크 환경에서는 DNS 해석/연결이 불안정할 수 있습니다. 커스텀 도메인을 사용하고 ICP 备案을 완료한 뒤, Alibaba Cloud OSS+CDN, Tencent Cloud COS+CDN 등 중국 본토에서 직접 접근 가능한 정적 호스팅에 배포하는 것을 권장합니다.
