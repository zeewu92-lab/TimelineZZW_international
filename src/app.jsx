    ...
    >
      ...
      >
        ...
        <div className={dock ? 'relative w-full h-full overflow-y-auto rounded-3xl p-5 flex flex-col' : 'relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col'} style={{ ...AUTH_GLASS, background: `rgba(255,255,255,${bgOpacity})`, zIndex: 1 }}>
          {/* 柔和背景光暈：呼應倒數卡片設計的兩顆模糊色塊，用事件本身的顏色調出淡淡光感 */}
          ...
          <div className="w-full flex items-start justify-between gap-2 relative" style={{ zIndex: 1 }}>
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className="font-bold truncate" style={{ color: INK, fontSize: 17, letterSpacing: '-0.01em' }}>{ev.title}</h3>
                <p className="text-xs truncate mt-0.5" style={{ color: INK_SOFT }}>{dateStr}</p>
              </div>
            </div>
            <button onClick={handleClose} aria-label={t.close} style={{ color: INK_SOFT, flexShrink: 0 }}><X size={18} /></button>
          </div>

          {/* 次要標籤：顏色標記／生日／關懷，移到標題下方一小行 */}
          ...
          {/* 中央超大剩餘天數，樣式比照倒數卡片設計：漸層大數字＋兩側分隔線的說明文字 */}
          <div className="flex flex-col items-center justify-center relative" style={{ zIndex: 1, padding: '22px 0' }}>
            <div
              style={{
                ...
                backgroundClip: 'text',
                color: 'transparent',
                filter: `drop-shadow(0 6px 16px ${colorHex(ev.colorId)}33)`,
              }}
            >
              {ev.diffDays === 0 ? '🎉' : Math.abs(ev.diffDays)}
            </div>
            <div className="flex items-center gap-4 mt-3" style={{ color: INK_SOFT, fontSize: 14, fontWeight: 500 }}>
          ...
          <div className="flex flex-col gap-3 relative" style={{ zIndex: 1 }}>
            {(showOrigDate || (ev.calendar && ev.calendar !== 'gregory')) && (
  ...
  function renderEventCard(ev) {
    return (
      <div key={ev.id} className="relative mb-6 pl-6">
        {/* 圓點指示器 */}
        <div className="absolute w-4 h-4 rounded-full" style={{ background: colorHex(ev.colorId), left: -25, top: 4, border: '3px solid var(--page-bg)', boxShadow: '0 0 0 1px var(--card-border)' }} />

        <div
          className="p-4 rounded-2xl relative group cursor-pointer"
          style={glass(ev.id === editingId ? { border: `1.5px solid ${ACCENT}` } : {})}
        ...
        >
          <div className="flex justify-between items-start mb-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={e => { e.stopPropagation(); setConfirmDeleteId(ev.id); }} aria-label={t.delete} className="p-2 rounded-lg transition-colors" style={{ color: DANGER }}
