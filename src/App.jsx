...
function CalendarDatePicker({ calendarId, isoDate, onChange, syncKey, lang, t }) {
  ...
  const yearListId = useId(); // 年份下拉清單改為可手動鍵入的 combobox，用 datalist 提供快速選項
      ...
      <div className="flex gap-1.5">
        ...
        <input
          ...
          onChange={e => {
            ...
            if (raw === '') return; // 使用者正在清空輸入框準備手動輸入新數字，先不更新，避免中途被重置成 NaN
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (el) {
        ...
        // 位置也對不上選單裡的任何一列。這裡直接判斷：選單開著時，只有屬於
        // 該選單容器內的國旗才繼續顯示，其餘一律暫時隱藏，等選單關閉再恢復。
        const withinOpenDropdown = !openDropdownEl || openDropdownEl.contains(el);
