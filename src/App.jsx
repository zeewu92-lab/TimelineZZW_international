...
import { Plus, Trash2, ChevronDown, ChevronLeft, X, MapPin, Check, Clock, Globe, Sun, Moon, Pencil, User, LogOut, Mail, Eye, EyeOff, Search, SlidersHorizontal, Share2 } from 'lucide-react';
import {
...
const STRINGS = {
  'zh-TW': {
    ...
    deleteLandmarkConfirmTitle: '刪除這個地標？', deleteLandmarkConfirmDesc: title => `如您未進行本機備份，此動作無法復原，確定要刪除「${title}」嗎？`, confirmDeleteLandmark: '刪除地標',
  ...
  en: {
    ...
    deleteLandmarkConfirmTitle: 'Delete this landmark?', deleteLandmarkConfirmDesc: title => `Unless you've made a local backup, this cannot be undone. Delete "${title}"?`, confirmDeleteLandmark: 'Delete landmark',
  ...
  ja: {
    ...
    deleteLandmarkConfirmTitle: 'このランドマークを削除しますか？', deleteLandmarkConfirmDesc: title => `ローカルバックアップを取っていない場合、この操作は取り消せません。「${title}」を削除しますか？`, confirmDeleteLandmark: 'ランドマークを削除',
  ...
  ko: {
    ...
    deleteLandmarkConfirmTitle: '이 랜드마크를 삭제할까요?', deleteLandmarkConfirmDesc: title => `로컬 백업을 하지 않았다면 이 작업은 되돌릴 수 없습니다. "${title}"을(를) 삭제하시겠습니까?`, confirmDeleteLandmark: '랜드마크 삭제',
...
function CurrentLocationClockModal({ clock, now, restClocks, lang, t, onClose, dock = false }) {
  const country = COUNTRIES.find(c => c.id === clock.countryId);
  const nameLabel = country ? country.name[lang] : clock.tz;
  const timeStr = new Intl.DateTimeFormat(LOCALE_MAP[lang], { timeZone: clock.tz, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(now);
  const offsetStr = getUtcOffset(clock.tz, now);
...
function WorldClockSection({ clocks, setClocks, lang, t, onHomeTzChange, homeTzId, setHomeTzId, part2Ref, part2Height, isDraggingWorldClock, isLargeScreen = false, clockModalOpen, setClockModalOpen }) {
  ...
  return (
    <div className="mb-2">
      ...
      {/* Part 2：其餘時區列表（以及尚未設定「目前位置」時的提示文字）。高度有上限（依畫面高度換算），
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, dock = false }) {
  ...
  return (
    <div id="timeline-section-root" className="flex-1 min-h-0 flex flex-col">
      ...
      {/* 刪除地標前的二次確認：跟帳號那邊「刪除帳號」用的是同一套風格
          （置中彈窗、AUTH_GLASS 毛玻璃卡片、標題用 DANGER 紅色、下面一個紅底確認鈕＋一個純文字取消鈕），
            ...
            <div className="flex flex-col gap-4 flex-shrink-0" style={{ width: 'clamp(300px, 34vw, 380px)' }}>
              <div className="flex-shrink-0">
                <WorldClockSection
                  ...
                  setClockModalOpen={setClockModalOpen}
                />
              </div>
              ...
              <div className="absolute inset-0" style={{ pointerEvents: desktopDetailOpen ? 'auto' : 'none' }}>
                {desktopDetailOpen && (
                  clockModalOpen && homeClock ? (
                    ...
                    <LandmarkDetailModal
                      dock
                      ev={desktopViewingEvent} lang={lang} t={t} isDark={isDark}
                      onClose={() => setViewingId(null)}
                      onSetBgImage={dataUrlOrNull => setDesktopEventBgImage(desktopViewingEvent.id, dataUrlOrNull)}
                      onSetBgOpacity={opacity => setDesktopEventBgOpacity(desktopViewingEvent.id, opacity)}
                    />
                  ) : null
                )}
              </div>
