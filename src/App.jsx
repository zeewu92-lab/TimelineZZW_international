...
const STRINGS = {
  'zh-TW': {
    ...
    backupCopyBtn: '複製備份文字', backupCopySuccess: '已複製到剪貼簿，可貼到記事本、Line、Email 等處保存。',
    backupPasteToggle: '或貼上備份文字還原', backupPastePlaceholder: '將備份文字貼在這裡',
    backupPasteImportBtn: '還原',
  ...
  en: {
    ...
    backupCopyBtn: 'Copy Backup Text', backupCopySuccess: 'Copied to clipboard — you can paste it into Notes, email, or a chat to yourself.',
    backupPasteToggle: 'Or restore by pasting backup text', backupPastePlaceholder: 'Paste your backup text here',
    backupPasteImportBtn: 'Restore',
  ...
  ja: {
    ...
    backupCopyBtn: 'バックアップ文字列をコピー', backupCopySuccess: 'クリップボードにコピーしました。メモ帳やメール、チャットなどに貼り付けて保存できます。',
    backupPasteToggle: 'またはテキストを貼り付けて復元', backupPastePlaceholder: 'バックアップのテキストをここに貼り付け',
    backupPasteImportBtn: '復元',
  ...
  ko: {
    ...
    backupCopyBtn: '백업 텍스트 복사', backupCopySuccess: '클립보드에 복사했습니다. 메모, 이메일, 채팅 등에 붙여넣어 보관할 수 있습니다.',
    backupPasteToggle: '또는 백업 텍스트를 붙여넣어 복원', backupPastePlaceholder: '백업 텍스트를 여기에 붙여넣으세요',
    backupPasteImportBtn: '복원',
...
function drawBrandWatermark(ctx, rightX, bottomY, colors) {
  ...
  const cy = bottomY - logoSize / 2;

  // logo：圓底 + 一條橫線（時間軸）+ 一個實心圓點（地標）
  ctx.beginPath();
  ctx.arc(x0 + logoSize / 2, cy, logoSize / 2, 0, Math.PI * 2);
  ...
  ctx.beginPath();
  ctx.moveTo(x0 + 9, cy);
  ctx.lineTo(x0 + logoSize - 9, cy);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(x0 + logoSize - 9, cy, 5, 0, Math.PI * 2);
  ctx.fillStyle = '#fff';
  ctx.fill();
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText(label, x0 + logoSize + gap, cy + 1);
}
...
async function buildEventCardCanvas(ev, lang, t, isDark) {
  ...
  if (ev.isCare) {
    drawPill(ctx, t.careLabel, contentX, cursorY - 34, { font: '700 28px "Noto Sans TC", "PingFang TC", sans-serif', textColor: colors.inkSoft, bgColor: colors.cardBorder, padX: 20, height: 52 });
    cursorY += 60;
  }
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity }) {
  ...
  return (
    <div
      ...
      onClick={handleClose}
    >
      {/* 高級感滑桿樣式：透明度滑桿改用細軌道＋白底描邊圓形滑塊，軌道填色隨數值即時漸層，
      ...
      <style>{`
        ...
        .premium-range::-webkit-slider-thumb {
          ...
          box-shadow: 0 2px 6px rgba(0,0,0,0.2), 0 0 0 4px rgba(108,123,224,0.12);
          transition: transform 0.15s ease, box-shadow 0.15s ease;
        }
        .premium-range::-webkit-slider-thumb:active {
          transform: scale(1.18);
          box-shadow: 0 3px 10px rgba(0,0,0,0.28), 0 0 0 6px rgba(108,123,224,0.16);
        }
        ...
        <div className="relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col" style={{ ...AUTH_GLASS, background: `rgba(255,255,255,${bgOpacity})`, zIndex: 1 }}>
          ...
          <div className="flex flex-col gap-3">
            ...
            {ev.isCare && (
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'var(--card-border)', color: INK_SOFT }}>{t.careLabel}</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-5 pt-4" style={{ borderTop: CARD_BORDER }}>
            ...
            {ev.bgImage && (
              <div
                style={{
                  maxHeight: showOpacityAdjust ? 72 : 0,
              ...
              >
                ...
                <input
                  ...
                  style={{ background: `linear-gradient(to right, ${ACCENT} ${Math.round(bgOpacity * 100)}%, var(--card-border) ${Math.round(bgOpacity * 100)}%)` }}
                />
              </div>
            )}
...
function AuthModal({ lang, t, user, onClose, backupData, onImportBackup }) {
  ...
  //
  // 另外提供「複製備份文字」＋「貼上文字還原」這組純文字管道，完全不牽涉檔案系統，
  // 兩端（複製／貼上）都能穩定運作，是最不容易踩雷的保底方案。
  ...
  async function handleCopyBackup() {
    ...
    try {
      ...
      setBackupMsg({ type: 'success', text: t.backupCopySuccess });
    } catch (err) {
      setBackupMsg({ type: 'error', text: t.backupImportError });
    }
  }
  ...
  function handlePasteImport() {
    ...
    if (parseAndImport(pasteText.trim())) {
      setPasteText('');
        ...
        {showPasteBox && (
          <div className="flex flex-col gap-2">
            <textarea
              ...
              placeholder={t.backupPastePlaceholder}
              rows={4}
              className="px-3 py-2.5 rounded-xl text-xs outline-none w-full resize-none"
              style={{ background: 'var(--input-bg)', border: CARD_BORDER, color: INK }}
            />
            <button
              type="button"
              ...
              style={{ background: MINT, color: '#fff', opacity: pasteText.trim() ? 1 : 0.6 }}
            >
              {t.backupPasteImportBtn}
            </button>
          </div>
        )}
