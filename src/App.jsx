...
async function buildEventCardCanvas(ev, lang, t, isDark) {
  ...
  if (ev.bgImage) {
    ...
    const glassCleared = ev.bgOverlayOpacity === -1;
    if (!glassCleared) {
      const opacity = ev.bgOverlayOpacity != null ? Math.max(0, Math.min(1, ev.bgOverlayOpacity)) : 0.75;
  ...
  return (
    <div
      ...
      style={{
        ...
        // 只裁切垂直方向（讓收合／展開的高度動畫照常運作），水平方向保持可見，
        // 這樣圓點指示器（靠負 left 位移「掛」在軸線上，會微幅超出這個容器左邊界）
        // 才不會被裁掉、被蓋住看不見。左側的軸線本身已改由外層容器統一畫一條貫穿到底，
        // 這裡不再各自畫線，避免跟外層的線之間出現斷開的間隙。
        overflowY: 'hidden',
        overflowX: 'visible',
    ...
    >
      <div ref={contentRef} className="relative" style={{ zIndex: 0 }}>
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto pb-6">
        {isSearching ? (
          searchResults.length === 0 ? (
            ...
            <div className="relative pl-4 border-l-2 ml-2" style={{ borderColor: '#000', zIndex: 0 }}>
