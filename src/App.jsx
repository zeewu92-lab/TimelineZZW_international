...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  // UI 滑桿反向映射：左端 = 遮罩 100%，右端 = 遮罩 0%。
  const overlaySliderValue = Math.round((1 - bgOpacity) * 100);
    ...
    >
      ...
      >
        ...
        <div className={dock ? 'relative w-full h-full overflow-y-auto rounded-3xl p-5 flex flex-col' : 'relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col'} style={{
          ...AUTH_GLASS,
          // backdropFilter / WebkitBackdropFilter 保持固定，滑桿只改這一層白色透明遮罩的 alpha。
          ...
          <div
            ...
            style={{
              ...
              overflow: 'hidden',
              transition: 'max-height 420ms cubic-bezier(0.22, 1, 0.36, 1), opacity 260ms ease, margin-top 360ms cubic-bezier(0.22, 1, 0.36, 1)',
            }}
          >
              ...
              {ev.bgImage && (
                <div
                  style={{
                    ...
                    overflow: 'hidden',
                    transition: 'max-height 380ms cubic-bezier(0.22, 1, 0.36, 1), opacity 240ms ease, margin-top 340ms cubic-bezier(0.22, 1, 0.36, 1)',
                  }}
                >
                  <div className="text-xs font-bold mb-2" style={{ color: INK_SOFT }}>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      // 0 = 左端／遮罩完全顯現；100 = 右端／遮罩完全消失
                      onSetBgOpacity(1 - sliderValue / 100);
                    }}
                    className="w-full premium-range"
                    aria-label={t.adjustBgOpacity}
                    style={{
                      // 軌道位置代表滑桿本身的位置；真正的遮罩透明度由上面的反向映射決定。
                      background: `linear-gradient(to right, ${ACCENT}99 0%, ${ACCENT}99 ${overlaySliderValue}%, rgba(127,127,127,0.18) ${overlaySliderValue}%, rgba(127,127,127,0.18) 100%)`,
                    }}
                  />
                </div>
              )}
...
function TimelineSection({
  ...
  const DELETE_MODAL_DURATION = 280;
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto pb-6">
        {isSearching ? (
          ...
          <>
            ...
            {pastEvents.length > 0 && (
              <div
                className="relative pl-4 border-l-2 ml-2 mb-2"
                style={{
                  borderColor: CARD_BORDER,
                  maxHeight: showPast ? 5000 : 0,
                  opacity: showPast ? 1 : 0,
                  transform: showPast ? 'translateY(0)' : 'translateY(-10px)',
                  overflow: 'hidden',
                  transformOrigin: 'top center',
                  transition:
                    'max-height 420ms cubic-bezier(0.22, 1, 0.36, 1), opacity 260ms ease, transform 420ms cubic-bezier(0.22, 1, 0.36, 1)',
                  willChange: 'max-height, opacity, transform',
                  pointerEvents: showPast ? 'auto' : 'none',
                }}
              >
      ...
      {showForm && createPortal(
        ...
        >
          <div
            ...
            onClick={e => e.stopPropagation()}
          >
            <div style={{ height: formHeight != null ? formHeight : 'auto', overflow: 'hidden', transition: 'height 380ms cubic-bezier(0.22, 1, 0.36, 1)' }}>
...
function MergeDialog({ t, onResolve }) {
  const isLargeScreen = useIsLargeScreen();
  const [phase, setPhase] = useState('enter');
  const MERGE_MODAL_DURATION = 300;
  useEffect(() => {
    const id = requestAnimationFrame(() => setPhase('shown'));
    return () => cancelAnimationFrame(id);
  }, []);
