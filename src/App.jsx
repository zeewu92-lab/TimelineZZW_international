...
async function buildEventCardCanvas(ev, lang, t, isDark) {
  ...
  const numberText = ev.mode === 'companion' ? String(Math.max(0, ev.elapsedDays ?? 0)) : ev.diffDays === 0 ? '🎉' : String(Math.abs(ev.diffDays));
  ctx.textAlign = 'center';
  ctx.textBaseline = 'alphabetic';
  if (ev.diffDays === 0) {
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  // 中央大數字實際顯示的內容與位數：當天（diffDays === 0）顯示慶祝 emoji，視同 1 位數的字級；
  ...
  const bigNumberDisplay = isCompanionMode
    ...
    : ev.diffDays === 0 ? '🎉' : String(Math.abs(ev.diffDays));
  // 注意：emoji 在 JS 字串裡是 surrogate pair，.length 會算成 2，不能直接拿字串長度當位數。
  const bigNumberFontSize = getBigNumberFontSize(ev.diffDays === 0 && !isCompanionMode ? 1 : bigNumberDisplay.length);
...
function TimelineSection({
  ...
  const DELETE_MODAL_DURATION = 110;
      ...
      {showForm && createPortal(
        ...
        >
          <div
            ...
            onClick={e => e.stopPropagation()}
          >
            <div style={{ height: formHeight != null ? formHeight : 'auto', overflow: 'hidden', transition: 'height 155ms cubic-bezier(0.34, 1.12, 0.64, 1)', willChange: 'height' }}>
...
function MergeDialog({ t, onResolve }) {
  const isLargeScreen = useIsLargeScreen();
  const [phase, setPhase] = useState('enter');
  const MERGE_MODAL_DURATION = 120;
  useEffect(() => {
    const id = requestAnimationFrame(() => setPhase('shown'));
    return () => cancelAnimationFrame(id);
  }, []);
