...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  // 拖動滑桿時，用 requestAnimationFrame 把實際送出去的 state 更新節流到最多每畫格一次，
  // 避免手指快速滑動時，原生 input 觸發的事件數量超過瀏覽器每秒能重繪的次數，
  // 累積出一堆排隊中的重新渲染，導致滑塊本身看起來卡頓、跟不上手指。
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative flex-1" style={{ paddingTop: 26 }}>
                      ...
                      <input
                        ...
                        onChange={e => {
                          // 用 requestAnimationFrame 節流：手指快速滑動時，實際送出去改變
                          // bgOpacity（觸發整個視窗重新渲染）的次數最多每畫格一次，滑塊本身
                          // 的顯示位置（value）仍然是原生、每次事件都即時更新，拖動起來更跟手。
                        ...
                        style={{
                          ...
                          // 不再是跟路標色無關的固定灰色。
                          background: `linear-gradient(to right, ${colorHex(ev.colorId)} 0%, ${colorHex(ev.colorId)} ${overlaySliderValue}%, ${colorHex(ev.colorId)}2A ${overlaySliderValue}%, ${colorHex(ev.colorId)}2A 100%)`,
