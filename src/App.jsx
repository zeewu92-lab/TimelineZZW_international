...
function CalendarDatePicker({ calendarId, isoDate, onChange, syncKey, lang, t }) {
  ...
  const [yearPickerKey, setYearPickerKey] = useState(0); // 每次選取後重新掛載，讓快速選單維持顯示成一個「▼」按鈕，不佔用顯示年份的空間
  ...
  const yearOptions = [];
  for (let y = yearRangeMax; y >= yearRangeMin; y--) yearOptions.push(y);
      ...
      <div className="flex gap-1.5">
        ...
        <select
          ...
          onChange={e => {
            ...
            setYearPickerKey(k => k + 1); // 選完立即重置回「▼」，維持成一個快速選單按鈕，不佔用顯示年份數字的空間
          }}
          aria-label={t.yearPickerLabel || (lang === 'en' ? 'Pick year' : '選擇年份')}
          className="px-1 py-2 rounded-lg text-sm outline-none shrink-0 w-8 text-center"
          style={SELECT_STYLE}
        >
