...
import { Plus, Trash2, ChevronDown, ChevronLeft, X, MapPin, Check, Clock, Globe, Sun, Moon, Pencil, User, LogOut, Mail, Eye, EyeOff, Search, SlidersHorizontal, Share2, Bell, BellOff } from 'lucide-react';
import {
...
const STRINGS = {
  'zh-TW': {
    ...
    adjustBgOpacity: '調節毛玻璃透明度', dragToAdjustOpacity: '滑動以調節透明度',
  ...
  en: {
    ...
    adjustBgOpacity: 'Adjust frosted-glass opacity', dragToAdjustOpacity: 'Slide to adjust opacity',
  ...
  ja: {
    ...
    adjustBgOpacity: 'すりガラスの透明度を調整', dragToAdjustOpacity: 'スライドして透明度を調整',
  ...
  ko: {
    ...
    adjustBgOpacity: '유리 질감 투명도 조절', dragToAdjustOpacity: '밀어서 투명도 조절',
...
function WorldClockSection({ clocks, setClocks, lang, t, onHomeTzChange, homeTzId, setHomeTzId, part2Ref, part2Height, isDraggingWorldClock, isLargeScreen = false, unlimitedHeight = false, clockModalOpen, setClockModalOpen }) {
  ...
  return (
    <div className="mb-2">
      ...
      {/* 大屏分欄模式下，這個視窗改由 App 嵌進右側面板顯示（見 App 內的桌面分欄版面），
          這裡就不再自己疊一層蓋住全畫面的彈窗，避免大屏下同時出現兩份 */}
      {!isLargeScreen && clockModalOpen && homeClock && (
...
// 品牌浮水印：一個自畫的簡易徽章（時間軸橫線＋地標圓點）＋「時光線」字樣，畫在卡片右下角
function drawBrandWatermark(ctx, rightX, bottomY, colors) {
  const label = '時光線';
  ...
  // logo：呼應 App icon 的星軌造型——深藍圓角方形徽章，一條弧線串起三個發光點
  ctx.save();
  const radius = logoSize * 0.28;
  roundRectPath(ctx, x0, topY, logoSize, logoSize, radius);
  const bgGrad = ctx.createLinearGradient(x0, topY, x0 + logoSize, topY + logoSize);
  bgGrad.addColorStop(0, '#283454');
  bgGrad.addColorStop(1, '#11151f');
  ctx.fillStyle = bgGrad;
  ctx.fill();
  ctx.clip();

  const p1 = { x: x0 + logoSize * 0.22, y: topY + logoSize * 0.74 }; // 左下：起點
  const p2 = { x: x0 + logoSize * 0.52, y: topY + logoSize * 0.5 };  // 中央：主節點
  const p3 = { x: x0 + logoSize * 0.8, y: topY + logoSize * 0.26 };  // 右上：終點

  ctx.strokeStyle = 'rgba(250,244,230,0.85)';
  ctx.lineWidth = Math.max(1.6, logoSize * 0.045);
  ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(p1.x, p1.y);
  ctx.quadraticCurveTo(p2.x, p1.y - (p1.y - p3.y) * 0.15, p2.x, p2.y);
  ctx.quadraticCurveTo(p2.x, p3.y + (p2.y - p3.y) * 0.15, p3.x, p3.y);
  ctx.stroke();

  [{ pt: p1, r: logoSize * 0.065 }, { pt: p2, r: logoSize * 0.095 }, { pt: p3, r: logoSize * 0.075 }].forEach(({ pt, r }) => {
    ctx.save();
    ctx.shadowColor = 'rgba(255,224,164,0.9)';
    ctx.shadowBlur = r * 3;
    ctx.beginPath();
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText(label, x0 + logoSize + gap, topY + logoSize / 2 + 1);
}
...
async function buildEventCardCanvas(ev, lang, t, isDark) {
  ...
  // 先量出標題要幾行、整體內容多高，才能決定畫布高度
  const measureCanvas = document.createElement('canvas');
  const mctx = measureCanvas.getContext('2d');
  const titleFont = '700 58px "Noto Sans TC", "PingFang TC", sans-serif';
  mctx.font = titleFont;
  const titleMaxWidth = w - EXPORT_PAD * 2 - 108;
  const titleLines = wrapCanvasText(mctx, ev.title || '', titleMaxWidth, 2);

  const headerH = Math.max(96, titleLines.length * 72);
  const dateBoxH = ev.calendar && ev.calendar !== 'gregory' ? 190 : (ev.showOrigDate ? 156 : 118);
  let h = EXPORT_PAD + headerH + 28 + dateBoxH + 28 + 78 + 40 + EXPORT_PAD + 60;
  h = Math.round(h);
  ...
  if (ev.bgImage) {
    try {
      ...
      drawImageCover(ctx, img, -20, -20, w + 40, h + 40);
      ctx.filter = 'none';
    } catch (err) {
      ctx.fillStyle = colors.cardBg;
      ctx.fillRect(0, 0, w, h);
    }
    const opacity = ev.bgOverlayOpacity != null ? ev.bgOverlayOpacity : 0.55;
    ctx.fillStyle = isDark ? `rgba(20,22,28,${Math.min(0.92, opacity + 0.15)})` : `rgba(255,255,255,${opacity})`;
    ctx.fillRect(0, 0, w, h);
  } else {
    ctx.fillStyle = colors.cardBg;
    ctx.fillRect(0, 0, w, h);
  ...
  // 標題列：icon + 標題 + 路標色 + （生日模式）
  ctx.textBaseline = 'alphabetic';
  ctx.textAlign = 'left';
  ctx.font = '58px "Noto Color Emoji", "Apple Color Emoji", sans-serif';
  ctx.fillText(ev.icon || '📌', contentX, cursorY + 52);
  const iconW = ctx.measureText(ev.icon || '📌').width + 20;

  ctx.font = titleFont;
  ctx.fillStyle = colors.ink;
  titleLines.forEach((line, i) => {
    ctx.fillText(line, contentX + iconW, cursorY + 52 + i * 68);
  });
  cursorY += headerH + 28;

  // 路標色 + 生日模式（放在標題下方，避免跟長標題擠在同一行造成截斷）
  let badgeX = contentX;
  ctx.font = '700 30px "Noto Sans TC", "PingFang TC", sans-serif';
  ctx.beginPath();
  ctx.arc(badgeX + 12, cursorY - 10, 12, 0, Math.PI * 2);
  ctx.fillStyle = colorHex(ev.colorId);
  ctx.fill();
  ctx.fillStyle = colors.inkSoft;
  ctx.fillText(t.markerColorLabel, badgeX + 34, cursorY);
  badgeX += 34 + ctx.measureText(t.markerColorLabel).width + 28;
  if (ev.isBirthday) {
    drawPill(ctx, t.birthdayLabel, badgeX, cursorY - 42, {
      font: '700 28px "Noto Sans TC", "PingFang TC", sans-serif', textColor: ACCENT, bgColor: `${ACCENT}26`, padX: 20, height: 52,
    });
  } else if (ev.isCare) {
    drawPill(ctx, t.careLabel, badgeX, cursorY - 42, {
      font: '700 28px "Noto Sans TC", "PingFang TC", sans-serif', textColor: colors.inkSoft, bgColor: colors.cardBorder, padX: 20, height: 52,
    });
  }
  cursorY += 30;

  // 日期卡片
  roundRectPath(ctx, contentX, cursorY, w - EXPORT_PAD * 2, dateBoxH, 28);
  ctx.fillStyle = colors.cardBg;
  ctx.fill();
  ctx.strokeStyle = colors.cardBorder;
  ctx.lineWidth = 2;
  ctx.stroke();
  let dy = cursorY + 56;
  ctx.font = '700 44px "Noto Sans TC", "PingFang TC", sans-serif';
  ctx.fillStyle = colors.ink;
  ctx.fillText(ev.dateStr, contentX + 32, dy);
  if (ev.showOrigDate) {
    dy += 44;
    ctx.font = '400 28px "Noto Sans TC", "PingFang TC", sans-serif';
    ctx.fillStyle = colors.inkSoft;
    ctx.fillText(`${t.originalDate}：${ev.origDateStr}`, contentX + 32, dy);
  }
  if (ev.calendar && ev.calendar !== 'gregory') {
    dy += ev.showOrigDate ? 40 : 44;
    ctx.font = '600 28px "Noto Sans TC", "PingFang TC", sans-serif';
    ctx.fillStyle = ACCENT;
    ctx.fillText(ev.altCalendarStr || '', contentX + 32, dy);
  }
  cursorY += dateBoxH + 28;

  // 倒數天數 / 年齡 / 週期膠囊
  let pillX = contentX;
  const pillFont = '700 38px "Noto Sans TC", "PingFang TC", sans-serif';
  const pillY = cursorY;
  const daysLabel = ev.diffDays === 0 ? t.today : ev.diffDays > 0 ? t.daysLeft(ev.diffDays) : t.daysAgo(Math.abs(ev.diffDays));
  pillX += drawPill(ctx, daysLabel, pillX, pillY, { font: pillFont, textColor: colorHex(ev.colorId), bgColor: `${colorHex(ev.colorId)}22`, height: 78 }) + 16;
  if (ev.age !== null && ev.age !== undefined) {
    pillX += drawPill(ctx, t.ageBadge(ev.age), pillX, pillY, { font: pillFont, textColor: colorHex(ev.colorId), bgColor: `${colorHex(ev.colorId)}22`, height: 78 }) + 16;
  }
  if (ev.repeat) {
    const repeatLabel = ev.repeatUnit === 'month' ? t.monthlyBadge(ev.repeatInterval) : t.yearlyBadge(ev.repeatInterval);
    pillX += drawPill(ctx, repeatLabel, pillX, pillY, { font: pillFont, textColor: colors.inkSoft, bgColor: colors.cardBorder, height: 78 }) + 16;
  }
  cursorY += 78 + 40;

  // 品牌浮水印
  drawBrandWatermark(ctx, w - EXPORT_PAD, h - EXPORT_PAD + 12, colors);
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, dock = false, closing = false }) {
  ...
  // 「調節毛玻璃透明度」面板的展開狀態：只有設定過自訂背景圖片時才有意義（見下方按鈕列），
  // 展開／收合都用 max-height + opacity 做過渡，讓視窗看起來是「絲滑地」變高／變矮，而不是瞬間跳動
  const [showOpacityAdjust, setShowOpacityAdjust] = useState(false);
  const bgOpacity = ev.bgOverlayOpacity != null ? ev.bgOverlayOpacity : 0.55;
  ...
  async function handleExport() {
    ...
    setExportError('');
    try {
      const exportEv = { ...ev, dateStr, origDateStr, showOrigDate, altCalendarStr };
  ...
  return (
    <div
      className={dock ? 'relative h-full w-full' : 'fixed inset-0 flex items-center justify-center px-6'}
      style={dock ? undefined : { zIndex: 200, background: shown ? 'rgba(0,0,0,0.4)' : 'rgba(0,0,0,0)', transition: `background ${CLOSE_DURATION}ms ease` }}
      onClick={dock ? undefined : handleClose}
    >
      {/* 高級感滑桿樣式：質感升級為「實體機械推子」——軌道做成毛玻璃凹槽（內陰影＋玻璃描邊），
          數值色透出並被 backdrop-filter 模糊成凹槽底部的柔光，滑塊做成金屬旋鈕／推子鍵帽
          （多層漸層模擬鋁材的頂部高光／底部暗面＋直紋防滑刻痕＋外部落陰讓它看起來浮凸在軌道上），
          原生 <input type="range"> 用 -webkit/-moz 的 pseudo-element 客製化外觀 */}
      <style>{`
        .premium-range {
        ...
        /* 軌道：毛玻璃凹槽——半透明白霧＋blur 讓底下數值色（由 inline style 提供）
           化開成一條柔光，外框亮邊＋內縮陰影營造「刻進面板裡」的立體感 */
        .premium-range::-webkit-slider-runnable-track {
          height: 10px;
          border-radius: 999px;
          background:
            linear-gradient(180deg, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0.15) 45%, rgba(0,0,0,0.06) 55%, rgba(255,255,255,0.4) 100%);
        ...
        .premium-range::-webkit-slider-thumb {
          ...
          border-radius: 5px;
          background:
            repeating-linear-gradient(90deg, rgba(0,0,0,0.18) 0px, rgba(0,0,0,0.18) 1px, transparent 1px, transparent 3.5px),
            linear-gradient(180deg, #ffffff 0%, #f1f2f5 10%, #d6d9de 46%, #b7bac2 54%, #e3e5ea 88%, #f4f5f8 100%);
          border: 1px solid rgba(120,124,134,0.45);
          box-shadow:
        ...
        .premium-range::-webkit-slider-thumb:active {
          ...
          box-shadow:
            ...
            0 0 0 5px rgba(108,123,224,0.14);
        }
      ...
      <div
        ...
        onClick={e => e.stopPropagation()}
      >
        {/* 自訂背景圖片：獨立一層，疊在毛玻璃卡片「後面」，跟卡片同尺寸、同圓角 */}
        {ev.bgImage && (
          <img
            src={ev.bgImage}
            alt=""
        ...
        <div className={dock ? 'relative w-full h-full overflow-y-auto rounded-3xl p-5 flex flex-col' : 'relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col'} style={{ ...AUTH_GLASS, background: `rgba(255,255,255,${bgOpacity})`, zIndex: 1 }}>
          <div className="w-full flex items-start justify-between mb-3 gap-2">
            <div className="flex items-center gap-2 min-w-0 flex-wrap">
              <span className="text-2xl flex-shrink-0">{ev.icon}</span>
              <h3 className="font-bold text-lg truncate" style={{ color: INK }}>{ev.title}</h3>
              <span className="inline-flex items-center gap-1 text-xs font-bold flex-shrink-0" style={{ color: INK_SOFT }}>
                <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: colorHex(ev.colorId) }} />
                {t.markerColorLabel}
              </span>
              {ev.isBirthday ? (
            ...
            <button onClick={handleClose} aria-label={t.close} style={{ color: INK_SOFT, flexShrink: 0 }}><X size={18} /></button>
          </div>

          <div className="flex flex-col gap-3">
            <div className="p-3 rounded-xl" style={{ background: CARD_BG, border: CARD_BORDER }}>
              <div className="text-base font-bold" style={{ color: INK }}>{dateStr}</div>
              {showOrigDate && (
                <div className="text-xs mt-1" style={{ color: INK_SOFT }}>{t.originalDate}：{origDateStr}</div>
              )}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {ev.repeat && (
                <div className="px-3 py-1 rounded-lg text-sm font-bold" style={{ background: 'var(--card-border)', color: INK_SOFT }}>
          ...
          {/* 自訂卡片背景：上傳／更換／移除，圖片會先在瀏覽器端等比縮小再存起來，避免佔用太多空間 */}
          <div className="mt-5 pt-4" style={{ borderTop: CARD_BORDER }}>
            <div className="text-xs font-bold mb-2" style={{ color: INK_SOFT }}>{t.customBgLabel}</div>
            <div className="flex items-center gap-2">
              <button
              ...
              {/* 匯出成圖片：跟「更換圖片」同一排的 icon 按鈕，點了展開下方的格式選擇＋分享面板 */}
              <button
                onClick={() => setShowExportPanel(v => !v)}
                aria-label={t.exportLabel}
                title={t.exportLabel}
                className="p-2 rounded-lg flex items-center justify-center flex-shrink-0 ml-auto"
                style={{ background: showExportPanel ? ACCENT : 'var(--card-border)', color: showExportPanel ? '#fff' : INK_SOFT, width: 36, height: 36 }}
              >
                <Share2 size={15} />
              </button>
            </div>
            ...
            {/* 調節毛玻璃透明度面板：只有設定過自訂背景圖片時才可能展開，
                用 max-height + opacity 過渡讓視窗高度變化看起來絲滑，而不是瞬間跳動 */}
            {ev.bgImage && (
              <div
                style={{
                  maxHeight: showOpacityAdjust ? 82 : 0,
                  opacity: showOpacityAdjust ? 1 : 0,
                  marginTop: showOpacityAdjust ? 14 : 0,
                  overflow: 'hidden',
                  transition: 'max-height 0.32s ease, opacity 0.28s ease, margin-top 0.32s ease',
                }}
              >
                <div className="text-xs font-bold mb-2" style={{ color: INK_SOFT }}>{t.dragToAdjustOpacity}</div>
                <input
                  type="range"
                  min={0}
                  max={100}
                  ...
                  style={{ background: `linear-gradient(to right, ${ACCENT}99 ${Math.round(bgOpacity * 100)}%, transparent ${Math.round(bgOpacity * 100)}%)` }}
                />
              </div>
            )}

            {/* 匯出成圖片面板：點右上角分享 icon 展開，跟「調節透明度」面板用同一種收合動畫 */}
            <div
              style={{
            ...
            >
              <div className="text-xs font-bold mb-2" style={{ color: INK_SOFT }}>{t.exportLabel}</div>
...
function TimelineSection({
  ...
  isLargeScreen = false, viewingId, setViewingId, onViewingEventChange,
}) {
  ...
  // 「地標詳情」視窗裡調整自訂背景的毛玻璃覆蓋透明度（0～1），同樣存進事件的 bgOverlayOpacity 欄位
  ...
  // 大屏分欄模式下，「地標詳情」實際上是在 App 那層渲染（嵌進右側面板），
  // 這裡把每次算好的 viewingEvent（含即時更新的倒數天數／歲數）回報上去，
  // App 才有內容可以顯示；非大屏模式完全不影響，App 收到也不會用到。
  useEffect(() => {
    if (onViewingEventChange) onViewingEventChange(viewingEvent);
  }, [viewingEvent, onViewingEventChange]);
  ...
  function renderEventCard(ev) {
    const isPast = ev.diffDays < 0;
    const isToday = ev.diffDays === 0;
    const displayDays = Math.abs(ev.diffDays);
    const accent = colorHex(ev.colorId);
    const statusLabel = isToday
      ? t.today
      : isPast
        ? t.daysAgo(displayDays)
        : t.daysLeft(displayDays);

    return (
      <div key={ev.id} className="relative mb-6 pl-6">
        {/* 時間軸指示點 */}
        <div
        ...
        <div
          ...
          style={{
            ...
            overflow: 'hidden',
            borderRadius: 28,
            border: ev.id === editingId ? `1.5px solid ${ACCENT}` : '1px solid rgba(255,255,255,0.78)',
            background: isDark
              ? 'linear-gradient(145deg, rgba(37,40,50,0.82), rgba(28,31,40,0.68))'
              : 'linear-gradient(145deg, rgba(255,255,255,0.82), rgba(247,248,252,0.64))',
            backdropFilter: 'blur(28px) saturate(150%)',
            WebkitBackdropFilter: 'blur(28px) saturate(150%)',
            boxShadow: isDark
              ? '0 14px 38px rgba(0,0,0,0.20), inset 0 1px 0 rgba(255,255,255,0.06)'
              : '0 14px 38px rgba(35,39,51,0.075), inset 0 1px 0 rgba(255,255,255,0.96)',
            transition: 'transform 0.18s ease, box-shadow 0.18s ease, border-color 0.18s ease',
          }}
          onMouseEnter={e => {
            e.currentTarget.style.transform = 'translateY(-2px)';
            e.currentTarget.style.boxShadow = isDark
              ? '0 18px 44px rgba(0,0,0,0.26), inset 0 1px 0 rgba(255,255,255,0.07)'
              : '0 18px 44px rgba(35,39,51,0.10), inset 0 1px 0 rgba(255,255,255,0.98)';
          }}
          onMouseLeave={e => {
        ...
        >
          ...
          <div
            ...
            style={{
              background: isDark
                ? 'linear-gradient(145deg, rgba(18,20,25,0.54), rgba(18,20,25,0.72))'
                : 'linear-gradient(145deg, rgba(255,255,255,0.58), rgba(255,255,255,0.78))',
              opacity: ev.bgImage ? (typeof ev.bgOverlayOpacity === 'number' ? Math.max(0.35, ev.bgOverlayOpacity) : 0.72) : 1,
            }}
          />
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                >
                  {ev.targetDate.toLocaleDateString(LOCALE_MAP[lang])}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <button
                ...
                aria-label={t.delete}
                className="p-2 rounded-xl transition-colors"
                style={{ color: DANGER, background: 'transparent' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,0,74,0.10)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
              >
                <Trash2 size={15} />
              </button>
          ...
          >
            <div
              style={{
                fontSize: 'clamp(92px, 15vw, 164px)',
                lineHeight: 0.82,
                fontWeight: 500,
                letterSpacing: '-0.075em',
                background: isPast
                  ? `linear-gradient(135deg, ${accent}AA, ${accent}70)`
                  : `linear-gradient(135deg, ${accent}DD, #8297F2)`,
                WebkitBackgroundClip: 'text',
                backgroundClip: 'text',
                color: 'transparent',
                filter: `drop-shadow(0 8px 20px ${accent}18)`,
            ...
            >
              <span style={{ width: 44, height: 1, background: 'var(--card-border)' }} />
              <span>{isToday ? t.today : isPast ? t.daysAgo(displayDays) : t.daysLeft(displayDays)}</span>
              <span style={{ width: 44, height: 1, background: 'var(--card-border)' }} />
            </div>
          </div>
          ...
          {/* 底部狀態資訊 */}
          <div
            className="absolute left-0 right-0 bottom-0 z-10 flex items-end justify-between gap-3"
            ...
            <div className="flex items-center gap-2 flex-wrap min-w-0">
              ...
              {ev.calendar && ev.calendar !== 'gregory' && (
                <span
                  ...
                  style={{ background: 'rgba(128,128,128,0.08)', color: INK_SOFT }}
                >
                  {formatAltCalendar(ev.targetDate, ev.calendar, lang, t)}
                </span>
              )}
  ...
  return (
    <div id="timeline-section-root" className="flex-1 min-h-0 flex flex-col">
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto pb-6">
        {isSearching ? (
          searchResults.length === 0 ? (
            ...
            <div className="relative pl-4 border-l-2 ml-2" style={{ borderColor: CARD_BORDER }}>
          ...
          <>
            ...
            {pastEvents.length > 0 && (
              <button
                ...
                style={{ color: INK_SOFT }}
              >
                <ChevronDown size={14} style={{ transform: showPast ? 'rotate(180deg)' : 'rotate(-90deg)', transition: 'transform 0.15s' }} />
                {t.pastLandmarks(pastEvents.length)}
              </button>
            )}
            {showPast && pastEvents.length > 0 && (
              <div className="relative pl-4 border-l-2 ml-2 mb-2" style={{ borderColor: CARD_BORDER }}>
      ...
      {showForm && createPortal(
        <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 200, background: 'rgba(0,0,0,0.4)' }}>
          <div className={`w-full ${isLargeScreen ? 'max-w-md' : 'max-w-sm'} max-h-[85vh] overflow-y-auto rounded-2xl p-4`} style={{ ...AUTH_GLASS, background: 'rgba(255,255,255,0.4)' }} onClick={e => e.stopPropagation()}>
            <div style={{ height: formHeight != null ? formHeight : 'auto', overflow: 'hidden', transition: 'height 0.3s ease' }}>
      ...
      {/* 地標詳情視窗：點一下時間軸卡片開啟，跟新增／編輯地標視窗一樣掛在 document.body 底下。
          大屏分欄模式下改由 App 嵌進右側面板顯示（見 App 內的桌面分欄版面與 onViewingEventChange 回報機制），
          這裡就不再自己疊一層蓋住全畫面的彈窗，避免大屏下同時出現兩份 */}
      {!isLargeScreen && viewingEvent && createPortal(
        <LandmarkDetailModal
      ...
      {confirmDeleteEvent && createPortal(
        <div
          ...
          style={{ zIndex: 205, background: 'rgba(0,0,0,0.4)' }}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setConfirmDeleteId(null)} aria-label={t.close} style={{ color: INK_SOFT }}><X size={18} /></button>
            </div>
            <p className="text-sm" style={{ color: INK }}>{t.deleteLandmarkConfirmDesc(confirmDeleteEvent.title)}</p>
            <div className="flex items-center gap-2.5">
              <button
                onClick={() => { deleteEvent(confirmDeleteEvent.id); setConfirmDeleteId(null); }}
...
function AuthModal({ lang, t, user, onClose, backupData, onImportBackup }) {
  ...
  if (user) {
    ...
    if (view === 'changePassword') {
      return (
        <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 200, background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
          ...
          <div className={`w-full ${isLargeScreen ? 'max-w-sm' : 'max-w-xs'} p-6 rounded-2xl flex flex-col gap-3`} style={AUTH_GLASS} onClick={e => e.stopPropagation()}>
            ...
            {providerId === 'password' && (
              <PasswordField
                ...
                onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); run(async () => { await deleteAccount(deletePassword); onClose(); }); } }}
                className="px-3 py-2.5 rounded-xl text-sm outline-none w-full"
                style={{ background: 'var(--input-bg)', border: CARD_BORDER, color: INK }}
              />
            )}
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 200, background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
        <div className={`w-full ${isLargeScreen ? 'max-w-sm' : 'max-w-xs'} p-6 rounded-2xl flex flex-col gap-4`} style={AUTH_GLASS} onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between">
...
function MergeDialog({ t, onResolve }) {
  const isLargeScreen = useIsLargeScreen();
  return (
    <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 210, background: 'rgba(0,0,0,0.5)' }}>
      <div className={`w-full ${isLargeScreen ? 'max-w-sm' : 'max-w-xs'} p-6 rounded-2xl flex flex-col gap-3`} style={AUTH_GLASS}>
        <h2 className="text-lg font-black" style={{ color: INK }}>{t.mergeTitle}</h2>
        <p className="text-sm" style={{ color: INK_SOFT }}>{t.mergeDesc}</p>
        <button onClick={() => onResolve('merge')} className="py-2.5 rounded-xl font-bold text-sm" style={{ background: MINT, color: '#fff' }}>
          {t.mergeOptionMerge}
        </button>
        <button onClick={() => onResolve('cloud')} className="py-2.5 rounded-xl font-bold text-sm" style={{ background: 'var(--input-bg)', border: CARD_BORDER, color: INK }}>
          {t.mergeOptionUseCloud}
        </button>
        <button onClick={() => onResolve('local')} className="py-2.5 rounded-xl font-bold text-sm" style={{ background: 'var(--input-bg)', border: CARD_BORDER, color: INK }}>
...
export default function App() {
  ...
  // clockModalOpen（目前位置時鐘詳情）與 viewingId（地標詳情）原本分別是 WorldClockSection／
  // TimelineSection 自己的 local state，現在提升到這一層——因為大屏分欄模式下，這兩個「詳情視窗」
  // 不再是蓋在畫面正中央的彈窗，而是要嵌進右側面板顯示，且時間軸本身也要挪到左側、疊在世界時鐘下方，
  // App 必須知道「現在有沒有詳情要看、要看哪一個」才能決定整個版面怎麼排。
  const isLargeScreen = useIsLargeScreen();
  const [clockModalOpen, setClockModalOpen] = useState(false);
  const [viewingId, setViewingId] = useState(null);
  // 地標詳情的實際內容（含即時更新的倒數天數／歲數等衍生欄位）算法比較複雜，
  // 沿用 TimelineSection 內部既有的 processedEvents 邏輯，不在這裡重複實作；
  // 改成讓 TimelineSection 算好之後透過 onViewingEventChange 回報上來即可
  const [desktopViewingEvent, setDesktopViewingEvent] = useState(null);
  // 大屏分欄模式下，任一個詳情視窗（時鐘或地標）開著就代表右側面板要顯示詳情，
  // 左側則要把時間軸移到世界時鐘下方一起顯示
  const desktopDetailOpen = isLargeScreen && (clockModalOpen || !!desktopViewingEvent);

  // 「目前位置時鐘詳情」跟「地標詳情」邏輯上互斥——同一時間只會有一張卡片是「使用者現在想看的」。
  // 這裡包一層：開時鐘詳情時順手把地標詳情關掉，反過來開地標詳情時也順手把時鐘詳情關掉，
  // 這樣不管手機或大屏，狀態永遠乾淨，也讓大屏分欄下面「右側該顯示哪張卡片」的判斷不會打結
  // （不然使用者在時鐘詳情開著時點了另一個地標，會因為時鐘詳情還開著而「切不過去」）。
  ...
  // ---- 大屏分欄右側面板：切換卡片時要先讓舊卡片播放關閉動畫，播完才換上新卡片 ----
  // clockModalOpen／desktopViewingEvent 兩者共同決定了「現在應該顯示哪張卡片」（desktopTarget），
  // 但右側面板實際「畫出來」的內容改用 desktopShown 這個獨立狀態，兩者刻意分開：
  // 目標一換成不同卡片（時鐘→地標、地標 A→地標 B、或反過來），desktopShown 不會立刻跟著換，
  // 而是先把 desktopPanelClosing 打開，讓目前顯示的卡片（透過 closing prop）播放跟使用者手動關閉
  // 一樣的收合動畫；等動畫時間（DOCK_SWITCH_MS，需跟 modal 內部的 CLOSE_DURATION 對齊）跑完，
  // 才真的把 desktopShown 換成新卡片，讓新卡片用原本的彈入動畫掛載——銜接起來就是「自動關閉舊視窗、
  // 彈出新視窗」，而不是內容瞬間跳掉。如果只是同一張卡片本身的內容更新（例如地標倒數天數、
  // 背景圖片變了），則直接同步內容，不重播任何動畫。
  const DOCK_SWITCH_MS = 220;
  const desktopTarget = !isLargeScreen ? null
    : clockModalOpen ? { kind: 'clock' }
    : desktopViewingEvent ? { kind: 'landmark', ev: desktopViewingEvent }
    : null;
  const [desktopShown, setDesktopShown] = useState(null);
  const [desktopPanelClosing, setDesktopPanelClosing] = useState(false);
  const desktopSwitchTimerRef = useRef(null);
  const desktopTargetKind = desktopTarget ? desktopTarget.kind : null;
  const desktopTargetLandmarkId = desktopTarget && desktopTarget.kind === 'landmark' ? desktopTarget.ev.id : null;
  useEffect(() => {
    const shownKind = desktopShown ? desktopShown.kind : null;
    const shownLandmarkId = desktopShown && desktopShown.kind === 'landmark' ? desktopShown.ev.id : null;
    const sameTarget = shownKind === desktopTargetKind && shownLandmarkId === desktopTargetLandmarkId;

    if (desktopSwitchTimerRef.current) { clearTimeout(desktopSwitchTimerRef.current); desktopSwitchTimerRef.current = null; }

    if (sameTarget) {
      // 同一張卡片：內容可能有更新（倒數天數、背景圖片……），直接同步，不用重播動畫
      if (desktopTarget) setDesktopShown(desktopTarget);
      setDesktopPanelClosing(false);
      return;
    }

    if (!shownKind || !desktopTargetKind) {
      // 從「沒有卡片」開新卡片，或從「有卡片」變成「沒有卡片」：都是單純的顯示／消失，直接套用即可
      setDesktopShown(desktopTarget);
      setDesktopPanelClosing(false);
      return;
    }

    // 換成了不同的卡片：先播放舊卡片的關閉動畫，播完後再掛上新卡片
    setDesktopPanelClosing(true);
    desktopSwitchTimerRef.current = setTimeout(() => {
      setDesktopShown(desktopTarget);
      setDesktopPanelClosing(false);
      desktopSwitchTimerRef.current = null;
    }, DOCK_SWITCH_MS);
  }, [desktopTargetKind, desktopTargetLandmarkId]);
  useEffect(() => () => { if (desktopSwitchTimerRef.current) clearTimeout(desktopSwitchTimerRef.current); }, []);
  ...
  // 大屏分欄模式下，「目前位置時鐘詳情」直接嵌進右側面板顯示（見下方 desktopDetailOpen 相關版面），
  // 用的時間需要每秒跳動才像個時鐘；上面的 nowTick 是給日期／時間軸倒數天數這類「不需要秒級精度」
  // 的地方用的，特意只每 30 秒更新一次以避免整個 App 每秒重渲染。這裡另外開一個只在該詳情視窗
  // 開著時才啟動的 1 秒計時器，兩者互不干擾，也不會讓 App 平常也跟著每秒重渲染
  const [clockModalNow, setClockModalNow] = useState(new Date());
  useEffect(() => {
    if (!clockModalOpen) return;
    setClockModalNow(new Date());
    const iv = setInterval(() => setClockModalNow(new Date()), 1000);
  ...
  // 「目前位置時鐘詳情」在大屏分欄模式下由這裡直接嵌進右側面板渲染（見下方分欄版面），
  // 所以需要在這一層也算出 homeClock／restClocks；算法跟 WorldClockSection 內部一樣簡單，
  // 直接沿用 clocks／homeTzId 這兩個本來就在 App 這一層的狀態即可，不需要另外提升更多邏輯上來
  const homeClock = clocks.find(c => c.id === homeTzId) || null;
  const restClocksForModal = clocks.filter(c => c.id !== homeTzId);

  // 「地標詳情」在大屏分欄模式下的背景圖片／透明度調整，直接沿用跟 TimelineSection 內部
  // 完全相同的寫法（更新 events 裡對應那一筆的 bgImage／bgOverlayOpacity 欄位）
  function setDesktopEventBgImage(id, dataUrlOrNull) {
    setEvents(prev => prev.map(e => (e.id === id ? { ...e, bgImage: dataUrlOrNull } : e)));
  }
  function setDesktopEventBgOpacity(id, opacity) {
    setEvents(prev => prev.map(e => (e.id === id ? { ...e, bgOverlayOpacity: opacity } : e)));
  }
  ...
  return (
    <>
      <div className="flex flex-col overflow-hidden" style={{ ...cssVars, height: '100dvh', background: 'transparent', fontFamily: "'Inter', sans-serif", transition: 'background 0.2s' }}>
        ...
        {/* Main Content */}
        {isLargeScreen ? (
          /* 折叠屏展開／平板／桌面等大屏：改成左右分欄——世界時鐘固定在左側，時間軸在右側獨立捲動
             （類似郵件 App 左右分欄）。一旦開啟「目前位置時鐘詳情」或「地標詳情」，時間軸會挪到
             左側、疊在世界時鐘下方（跟手機版的上下疊放方式相同），右側則改成顯示剛開啟的詳情面板，
             並帶一個從右側「彈射」滑入的進場動畫（詳情面板內部已經處理，見 CurrentLocationClockModal／
             LandmarkDetailModal 的 dock 模式）。世界時鐘與時間軸兩個元件本身都只掛載一份，
             只是依 desktopDetailOpen 決定要放進左側或右側的容器，避免同一份資料同時存在兩份實例。 */
          <main className="px-6 md:px-10 max-w-[1180px] mx-auto w-full flex-1 min-h-0 flex flex-row gap-6 pb-4">
            <div className="flex flex-col gap-4 flex-shrink-0" style={{ width: 'clamp(300px, 34vw, 380px)' }}>
              <div className="flex-shrink-0">
                <WorldClockSection
                  clocks={clocks}
                  setClocks={setClocks}
                  ...
                  isLargeScreen
                  unlimitedHeight={!desktopDetailOpen}
                  clockModalOpen={clockModalOpen}
                  setClockModalOpen={openClockModalSafe}
                />
              </div>
              {/* 時間軸挪到左側、疊在世界時鐘下方：只有詳情面板開著時才顯示這裡的這一份，
                  用 opacity／transform 做淡入＋輕微下滑的過渡，避免版面切換時太過突然 */}
              <div
                className="min-h-0"
                style={{
                  flex: desktopDetailOpen ? '1 1 auto' : '0 0 0px',
                  opacity: desktopDetailOpen ? 1 : 0,
                  overflow: 'hidden',
                  transform: desktopDetailOpen ? 'translateY(0px)' : 'translateY(-6px)',
                  transition: 'opacity 0.3s ease, transform 0.3s ease, flex-basis 0.3s ease',
                }}
              >
                {desktopDetailOpen && (
                  <div className="h-full overflow-y-auto">
                    <TimelineSection
                      ...
                      onViewingEventChange={setDesktopViewingEvent}
                    />
                  </div>
                )}
              </div>
            ...
            {/* 右側面板：正常狀態顯示時間軸（獨立捲動），開啟詳情時改顯示詳情面板本身 */}
            <div className="flex-1 min-h-0 relative overflow-hidden">
              <div
                className="absolute inset-0"
                style={{
                  opacity: desktopDetailOpen ? 0 : 1,
                  transform: desktopDetailOpen ? 'scale(0.98)' : 'scale(1)',
                  transition: 'opacity 0.25s ease, transform 0.25s ease',
                  pointerEvents: desktopDetailOpen ? 'none' : 'auto',
                }}
              >
                {!desktopDetailOpen && (
                  <div className="h-full overflow-y-auto">
              ...
              <div className="absolute inset-0" style={{ pointerEvents: desktopDetailOpen ? 'auto' : 'none' }}>
                {desktopShown && (
                  desktopShown.kind === 'clock' && homeClock ? (
                    <CurrentLocationClockModal
                      key="desktop-clock"
                      dock
                      closing={desktopPanelClosing}
                      clock={homeClock} now={clockModalNow} lang={lang} t={t}
                      restClocks={restClocksForModal}
                      onClose={() => setClockModalOpen(false)}
                    />
                  ) : desktopShown.kind === 'landmark' ? (
                    <LandmarkDetailModal
                      key={`desktop-landmark-${desktopShown.ev.id}`}
                      dock
                      closing={desktopPanelClosing}
                      ev={desktopShown.ev} lang={lang} t={t} isDark={isDark}
                      onClose={() => setViewingId(null)}
                      onSetBgImage={dataUrlOrNull => setDesktopEventBgImage(desktopShown.ev.id, dataUrlOrNull)}
                      onSetBgOpacity={opacity => setDesktopEventBgOpacity(desktopShown.ev.id, opacity)}
                    />
                  ) : null
                )}
              </div>
