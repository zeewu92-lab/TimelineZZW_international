...
async function buildEventCardCanvas(ev, lang, t, isDark) {
  ...
  // 中文版另外換成「DotGothic16」點陣字體＋放大字級；其餘語言字體不變，僅英文放大（日文／韓文字級也不變）。
  ...
  if (isTodayTextMessage) {
    const todayFontFamily = isZh ? '"DotGothic16", "Noto Sans TC", "PingFang TC", sans-serif' : '"Noto Sans TC", "PingFang TC", sans-serif';
    const todayFontSize = isZh || lang === 'en' ? 116 : 100; // 中文／英文在 canvas 放大，日文／韓文維持原本大小
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  // 中文版當天文字改用「DotGothic16」點陣字體，字級也比其他語言再放大一些；其他語言不換字體，
  // 沿用原本的行為（跟著使用者選的數字字體走），卡片內字級也維持原本大小
  // （英文只在下面 canvas 匯出時放大，詳情卡片本身不放大）。
  const bigNumberFontSize = isTodayTextMessage
    ? (isZh ? 60 : 52)
    : getBigNumberFontSize(bigNumberDisplay.length);
  const todayTextFontFamily = isZh ? "'DotGothic16', 'Noto Sans TC', sans-serif" : numberFontFamily;
...
function TimelineSection({
  ...
  const DELETE_MODAL_DURATION = 80;
      ...
      {showForm && createPortal(
        ...
        >
          <div
            ...
            onClick={e => e.stopPropagation()}
          >
            <div style={{ height: formHeight != null ? formHeight : 'auto', overflow: 'hidden', transition: 'height 220ms cubic-bezier(0.22, 1, 0.36, 1)', willChange: 'height' }}>
...
export default function App() {
  ...
  // 同時預先載入「DotGothic16」：中文版當天卡片中央的「生日快樂！」／「一切順利！」字樣專用字體，
  // 一樣提早載入，避免使用者剛好在當天打開卡片、或直接匯出圖片時字體還沒到位而 fallback 成系統字體。
  useEffect(() => {
    const defaultFont = NUMBER_FONTS.find(f => f.id === 'inter');
    if (defaultFont) ensureGoogleFontLoaded(defaultFont.googleFont);
    ensureGoogleFontLoaded('DotGothic16');
  }, []);
