...
function AuthModal({ lang, t, user, onClose, backupData, onImportBackup }) {
  ...
  // 直接把目前的 clocks／events／lang／isDark／customIcons 整包輸出成一個 JSON 檔案讓使用者自行保存，
  // 需要的時候再挑檔案匯入、走跟「以雲端資料覆蓋」完全相同的 applyCloudData 邏輯還原回來。
  const importFileRef = useRef(null);
  const [backupMsg, setBackupMsg] = useState(null); // { type: 'success' | 'error', text }

  function handleExportBackup() {
    const payload = { ...backupData, exportedAt: Date.now() };
