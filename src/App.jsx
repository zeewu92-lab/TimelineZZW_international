...
// 中曆需要「長格式」月份名稱（例如「正月」「閏二月」）才能正確分辨閏月，數字格式無法分辨。
function chineseMonthInfo(date) {
  try {
    const dtf = new Intl.DateTimeFormat('zh-TW-u-ca-chinese', { year: 'numeric', month: 'long', day: 'numeric' });
    const o = {};
    dtf.formatToParts(date).forEach(p => (o[p.type] = p.value));
    return { year: parseInt(o.relatedYear || o.year), month: o.month, day: parseInt(o.day) };
  } catch (e) { return null; }
}
...
function CalendarDatePicker({ calendarId, isoDate, onChange, syncKey, lang, t }) {
  ...
  // year 由外部（切換曆法、切換年號、點選快速選單）變動時，同步更新輸入框顯示文字；
  // 使用者手動刪空輸入框時 year 不會跟著變動，因此不會覆蓋掉使用者正在清空的狀態
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (calendarId === 'chinese') {
      ...
      if (info) { setYear(info.year); setMonth(info.month); setDay(info.day); }
    } else if (calendarId === 'japanese') {
      const p = calNumericParts(base, 'japanese');
      const e = getJapaneseEra(base);
      if (p && e) { setEra(e.id); setYear(e.year); setMonth(p.month); setDay(p.day); }
    } else {
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex gap-1.5">
        {calendarId === 'japanese' && (
          <select
            value={era}
            onChange={e => { const nextEra = e.target.value; const maxY = japaneseEraYearMax(nextEra); setEra(nextEra); setYear(y => Math.min(y || 1, maxY)); }}
        ...
        <div className="relative flex-1 min-w-0">
          <input
            ...
            onChange={e => {
              ...
              setYearText(raw); // 一律先更新輸入框顯示文字，允許使用者刪到完全清空
            ...
            onBlur={() => {
              ...
              if (yearText === '' || Number.isNaN(parseInt(yearText, 10))) setYearText(String(year));
            }}
            className={`${SELECT_CLASS} w-full pr-6`} style={SELECT_STYLE}
          />
          {/* 疊在輸入框右側、透明不可見的原生選單：點下去會彈出年份清單，且開啟時會直接定位在目前的年份（而非清單最上方），
              視覺上只露出一個小箭頭圖示，作為「用選單挑年份」的入口，跟左側可直接手動輸入的欄位相輔相成 */}
          <select
            value={year}
            onChange={e => {
          ...
          <svg className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 w-2.5 h-2.5" viewBox="0 0 10 10" fill="none">
            <path d="M2 3.5L5 6.5L8 3.5" stroke={INK} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" opacity="0.55" />
          </svg>
        </div>
        <select
          value={month}
...
function FlagPortal({ flag, className, style }) {
...
function ClockRow({ clock, now, selectMode, selected, onLongPress, onTap, lang, t, compact, isHome, homeTz, hero }) {
  ...
  const handleClick = () => { if (firedRef.current) { firedRef.current = false; return; } onTap(clock.id); };
        ...
        >
          <div className="flex justify-between items-start mb-1">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => startEdit(ev)} aria-label={t.edit} className="p-2 rounded-lg transition-colors" style={{ color: INK_SOFT }}
                onMouseEnter={e => (e.currentTarget.style.background = 'var(--card-border)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                <Pencil size={15} />
              </button>
              <button onClick={() => deleteEvent(ev.id)} aria-label={t.delete} className="p-2 rounded-lg transition-colors" style={{ color: DANGER }}
