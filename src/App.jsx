...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  // 面板一打開才動態載入本卡片會用到的 Google Fonts（系統圓體／Quicksand 不需要，其餘幾款才需要）
  useEffect(() => {
    if (!showCustomizePanel) return;
    NUMBER_FONTS.forEach(f => ensureGoogleFontLoaded(f.googleFont));
  }, [showCustomizePanel]);
  const numberFontId = ev.numberFont || 'system';
                ...
                {NUMBER_FONTS.map(f => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ fontFamily: f.family, fontSize: 22, fontWeight: 700, lineHeight: 1, color: INK }}>88</span>
...
export default function App() {
  ...
  function setDesktopEventNumberFont(id, fontId) {
    setEvents(prev => prev.map(e => (e.id === id ? { ...e, numberFont: fontId } : e)));
  }
                ...
                {desktopShown && (
                  desktopShown.kind === 'clock' && homeClock ? (
                    ...
                    <LandmarkDetailModal
                      ...
                      onSetNumberFont={fontId => setDesktopEventNumberFont(desktopShown.ev.id, fontId)}
                    />
                  ) : null
                )}
              </div>
