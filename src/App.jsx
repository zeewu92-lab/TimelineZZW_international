      ...
      <div className="flex gap-1.5">
        ...
        <div style={{ position: 'relative', flex: '1 1 0%', minWidth: 0 }}>
          <input
            ...
            className={SELECT_CLASS} style={{ ...SELECT_STYLE, width: '100%', paddingRight: '1.5rem' }}
          />
          {/* 疊在輸入框右側、透明不可見的原生選單：點下去會彈出年份清單，且開啟時會直接定位在目前的年份（而非清單最上方），
              視覺上只露出一個小箭頭圖示，跟左側可直接手動輸入的欄位合併成同一個方框，不會變成兩個重複顯示數字的框。
              這裡改用行內樣式（inline style）而非 Tailwind class 來定位，避免因為專案的 CSS 沒有預先產生這些少見的
              class（例如 opacity-0、inset-y-0）而導致箭頭整個不顯示 */}
          <select
            value={year}
            onChange={e => {
