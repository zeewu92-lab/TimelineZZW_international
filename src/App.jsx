...
// 驗證並解析「本機備份 .json」的內容是否符合預期格式，回傳 parse 好的物件；格式不對或不是合法 JSON 則回傳 null。
// 這個函式同時給「匯入備份」按鈕（AuthModal 裡的 parseAndImport）與 File Handling API（作業系統「以 App 開啟」
// .json 檔時觸發的 launchQueue consumer）共用，避免兩邊各寫一份驗證邏輯、之後改格式時容易漏改其中一處。
function parseBackupPayload(jsonText) {
  try {
...
function CurrentLocationClockModal({ clock, now, restClocks, lang, t, onClose }) {
  const country = COUNTRIES.find(c => c.id === clock.countryId);
  const nameLabel = country ? country.name[lang] : clock.tz;
  const timeStr = new Intl.DateTimeFormat(LOCALE_MAP[lang], { timeZone: clock.tz, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(now);
  const offsetStr = getUtcOffset(clock.tz, now);
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col items-center"
        style={{
          ...AUTH_GLASS,
          opacity: shown ? 1 : 0,
...
function WorldClockSection({ clocks, setClocks, lang, t, onHomeTzChange, homeTzId, setHomeTzId, part2Ref, part2Height, isDraggingWorldClock }) {
  ...
  function openClockModal() { setShowClockModal(true); }
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity }) {
  ...
  return (
    ...
    >
      ...
      `}</style>
      <div
        className="relative w-full max-w-sm max-h-[85vh] rounded-3xl"
        style={{
          opacity: shown ? 1 : 0,
          transform: shown ? 'scale(1) translateY(0px)' : 'scale(0.92) translateY(14px)',
          transition: `opacity ${CLOSE_DURATION}ms ease, transform ${CLOSE_DURATION}ms cubic-bezier(0.34, 1.56, 0.64, 1)`,
      ...
      >
        ...
        <div className="relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col" style={{ ...AUTH_GLASS, background: `rgba(255,255,255,${bgOpacity})`, zIndex: 1 }}>
...
function TimelineSection({ events, setEvents, lang, t, now, isDark, customIcons, setCustomIcons, onHeaderDragStart, onHeaderDragMove, onHeaderDragEnd }) {
  ...
  const [viewingId, setViewingId] = useState(null); // 目前開啟「地標詳情」視窗的事件 id
        ...
        >
          <div className="flex justify-between items-start mb-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={e => { e.stopPropagation(); deleteEvent(ev.id); }} aria-label={t.delete} className="p-2 rounded-lg transition-colors" style={{ color: DANGER }}
    ...
    <div id="timeline-section-root" className="flex-1 min-h-0 flex flex-col">
      ...
      {showForm && createPortal(
        <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 200, background: 'rgba(0,0,0,0.4)' }}>
          <div className="w-full max-w-sm max-h-[85vh] overflow-y-auto rounded-2xl p-4" style={{ ...AUTH_GLASS, background: 'rgba(255,255,255,0.4)' }} onClick={e => e.stopPropagation()}>
...
function AuthModal({ lang, t, user, onClose, backupData, onImportBackup }) {
  ...
  //
  // Android 上單純觸發 <a download> 存成 .json 檔，使用者常常找不到檔案存去哪了（下載資料夾要另外搜尋，
  ...
  function parseAndImport(jsonText) {
    const data = parseBackupPayload(jsonText);
    if (!data) {
  ...
  async function handleExportBackup() {
    ...
    const filename = `timeline-backup-${dateStr}.json`;
  ...
  if (user) {
    ...
    if (view === 'changePassword') {
      return (
        <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 200, background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
          <div className="w-full max-w-xs p-6 rounded-2xl flex flex-col gap-3" style={AUTH_GLASS} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 200, background: 'rgba(0,0,0,0.4)' }} onClick={onClose}>
        <div className="w-full max-w-xs p-6 rounded-2xl flex flex-col gap-4" style={AUTH_GLASS} onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between">
...
function MergeDialog({ t, onResolve }) {
  return (
    <div className="fixed inset-0 flex items-center justify-center px-6" style={{ zIndex: 210, background: 'rgba(0,0,0,0.5)' }}>
      <div className="w-full max-w-xs p-6 rounded-2xl flex flex-col gap-3" style={AUTH_GLASS}>
...
export default function App() {
  ...
  // File Handling API：使用者在作業系統裡直接用「開啟檔案」／雙擊 .json 備份檔、
  // 或對著已安裝的 App 圖示把 .json 檔拖進去時，瀏覽器會啟動這個 PWA 並把檔案透過
  ...
  useEffect(() => {
    ...
    window.launchQueue.setConsumer(async (launchParams) => {
      ...
      try {
        ...
        if (!data) {
          setFileHandlerMsg({ type: 'error', text: t.backupImportError });
          return;
        }
        applyCloudData(data);
        setFileHandlerMsg({ type: 'success', text: t.backupImportSuccess });
      } catch (err) {
        setFileHandlerMsg({ type: 'error', text: t.backupImportError });
      }
    });
  ...
  return (
    <>
      <div className="flex flex-col overflow-hidden" style={{ ...cssVars, height: '100dvh', background: 'transparent', fontFamily: "'Inter', sans-serif", transition: 'background 0.2s' }}>
        ...
        {/* Main Content — flex column：上半部（世界時鐘）固定，下半部（時間軸）自己有獨立捲動容器 */}
