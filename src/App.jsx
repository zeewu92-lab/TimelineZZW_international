  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 唯一受滑桿控制的白色遮罩。
            右端 100% = alpha 0，遮罩完全消失；左端 0% = alpha 1，遮罩完全顯現。 */}
        {ev.bgImage && (
          <div
            className="absolute inset-0 rounded-3xl pointer-events-none"
            style={{
        ...
        <div className={dock ? 'relative w-full h-full overflow-y-auto rounded-3xl p-5 flex flex-col' : 'relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-5 flex flex-col'} style={{
          ...
          <div
            ...
            style={{
              ...
              overflow: 'hidden',
              transition: 'max-height 280ms cubic-bezier(0.22, 1, 0.36, 1), opacity 180ms ease, margin-top 240ms cubic-bezier(0.22, 1, 0.36, 1)',
            }}
          >
              ...
              {ev.bgImage && (
                <div
                  style={{
                    ...
                    overflow: 'hidden',
                    transition: 'max-height 260ms cubic-bezier(0.22, 1, 0.36, 1), opacity 180ms ease, margin-top 220ms cubic-bezier(0.22, 1, 0.36, 1)',
                  }}
                >
...
function TimelineSection({
  ...
  const DELETE_MODAL_DURATION = 200;
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto pb-6">
        {isSearching ? (
          ...
          <>
            ...
            {pastEvents.length > 0 && (
              <div
                className="relative ml-2 mb-2"
                style={{
                  display: 'grid',
                  gridTemplateRows: showPast ? '1fr' : '0fr',
                  opacity: showPast ? 1 : 0,
                  transform: showPast ? 'translateY(0)' : 'translateY(-8px)',
                  transition:
                    'grid-template-rows 300ms cubic-bezier(0.22, 1, 0.36, 1), opacity 190ms ease, transform 300ms cubic-bezier(0.22, 1, 0.36, 1)',
              ...
              >
                <div
                  className="min-h-0 overflow-hidden pl-4 border-l-2"
      ...
      {showForm && createPortal(
        ...
        >
          <div
            ...
            onClick={e => e.stopPropagation()}
          >
            <div style={{ height: formHeight != null ? formHeight : 'auto', overflow: 'hidden', transition: 'height 260ms cubic-bezier(0.22, 1, 0.36, 1)' }}>
...
function MergeDialog({ t, onResolve }) {
  const isLargeScreen = useIsLargeScreen();
  const [phase, setPhase] = useState('enter');
  const MERGE_MODAL_DURATION = 220;
  useEffect(() => {
    const id = requestAnimationFrame(() => setPhase('shown'));
    return () => cancelAnimationFrame(id);
  }, []);
