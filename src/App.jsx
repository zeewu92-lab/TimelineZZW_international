...
async function buildEventCardCanvas(ev, lang, t, isDark) {
  ...
  if (ev.bgImage) {
    ...
    const glassCleared = ev.bgOverlayOpacity === -1;
    if (!glassCleared) {
      const opacity = ev.bgOverlayOpacity != null ? Math.max(0, Math.min(1, ev.bgOverlayOpacity)) : 0.55;
...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  // bgOverlayOpacity 0～1 代表遮罩本身的不透明度；-1 是一個保留值，代表「清除玻璃效果」。
  // 這樣可以在不新增資料欄位的前提下保存「原圖模式」，也能兼容既有事件資料。
  const glassCleared = ev.bgOverlayOpacity === -1;
  const bgOpacity = glassCleared ? 0 : Math.max(0, Math.min(1, ev.bgOverlayOpacity != null ? ev.bgOverlayOpacity : 0.55));
  const OVERLAY_ZERO_POINT = 100;
  const SLIDER_MAX = 120;
  const SNAP_START = 94;
  const originalImageLabel = lang === 'zh-TW' ? '原圖' : lang === 'ja' ? '原画像' : lang === 'ko' ? '원본' : 'Original';
  // 0～100：控制遮罩顯隱；100～120：進入「原圖」區域。
  const overlaySliderValue = glassCleared ? SLIDER_MAX : Math.round((1 - bgOpacity) * OVERLAY_ZERO_POINT);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 玻璃效果層：正常模式固定輕度模糊；進入滑桿最右側的「原圖區」後，整層完全移除。 */}
        ...
        {/* 唯一受滑桿控制的白色遮罩。100 是「遮罩完全消失」的臨界點；超過 100 直接進入原圖模式。 */}
        {ev.bgImage && !glassCleared && (
          <div
            className="absolute inset-0 rounded-3xl pointer-events-none"
            style={{
            ...
            <div className="pb-4" style={{ borderBottom: CARD_BORDER }}>
              ...
              {ev.bgImage && (
                ...
                >
                  ...
                  <div className="relative w-full" style={{ paddingTop: 2 }}>
                    {/* 新滑桿比原本多出 20% 的右側行程：100% 是「遮罩完全消失」的吸附臨界點，100～120% 是「清除玻璃效果／原圖」區。 */}
                    <input
                      type="range"
                      min={0}
                      max={SLIDER_MAX}
                      ...
                      onChange={e => {
                        ...
                        // 接近臨界點時自動吸附到 100。只有真正繼續向右拖過臨界點，才進入原圖模式。
                        if (sliderValue >= SNAP_START && sliderValue <= OVERLAY_ZERO_POINT) {
                          sliderValue = OVERLAY_ZERO_POINT;
                        }
                        if (sliderValue > OVERLAY_ZERO_POINT) {
                          onSetBgOpacity(-1);
                        } else {
                          onSetBgOpacity(1 - sliderValue / OVERLAY_ZERO_POINT);
                        }
                      }}
                      className="w-full premium-range"
                      aria-label={t.adjustBgOpacity}
                      style={{
                        // 前 100/120 為遮罩控制區，後 20/120 為原圖區；臨界點用細刻線標示。
                        background: `linear-gradient(to right, ${ACCENT}99 0%, ${ACCENT}99 ${(OVERLAY_ZERO_POINT / SLIDER_MAX) * 100}%, rgba(127,127,127,0.18) ${(OVERLAY_ZERO_POINT / SLIDER_MAX) * 100}%, rgba(127,127,127,0.18) 100%)`,
                      }}
                    />
                    <span
                      aria-hidden="true"
                      style={{
                        position: 'absolute',
                        left: `${(OVERLAY_ZERO_POINT / SLIDER_MAX) * 100}%`,
                        top: 0,
                        width: 1,
                        height: 18,
                    ...
                    <span
                      ...
                      style={{
                        ...
                        fontSize: 10,
                        lineHeight: 1,
                        color: glassCleared ? ACCENT : INK_SOFT,
                        fontWeight: 700,
                        pointerEvents: 'none',
                      }}
                    >{originalImageLabel}</span>
                  </div>
                </div>
              )}
            </div>
...
function PastEventsAnimatedSection({ show, events, renderEventCard }) {
  ...
  return (
    <div
      className="relative ml-2 mb-2 overflow-hidden"
      style={{
        height,
        opacity: show ? 1 : 0,
        transform: show ? 'translateY(0)' : 'translateY(-6px)',
        pointerEvents: show ? 'auto' : 'none',
        transition: 'height 160ms cubic-bezier(0.2, 0.8, 0.2, 1), opacity 110ms ease, transform 160ms cubic-bezier(0.2, 0.8, 0.2, 1)',
    ...
    >
      <div
        ...
        style={{ borderColor: 'var(--card-border)', position: 'relative', zIndex: 0 }}
      >
...
function TimelineSection({
  ...
  function renderEventCard(ev) {
    return (
      <div key={ev.id} className="relative mb-6 pl-6" style={{ zIndex: 10 }}>
      ...
      <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto pb-6">
        {isSearching ? (
          searchResults.length === 0 ? (
            ...
            <div className="relative pl-4 border-l-2 ml-2" style={{ borderColor: 'var(--card-border)', zIndex: 0 }}>
