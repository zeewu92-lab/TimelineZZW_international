...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  const bigNumberDisplay = ev.diffDays === 0 ? '🎉' : String(Math.abs(ev.diffDays));
  // 注意：emoji 在 JS 字串裡是 surrogate pair，.length 會算成 2，不能直接拿字串長度當位數，
  // 所以慶祝 emoji 一律當成「1 位數」等級處理，其餘情況才用實際數字字串的長度。
  const bigNumberFontSize = getBigNumberFontSize(ev.diffDays === 0 ? 1 : bigNumberDisplay.length);
...
function TimelineSection({
  ...
  function startEdit(ev) {
    ...
    setEventMode(eventModeFromEv(ev));
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: active ? '1px solid transparent' : '1px solid rgba(255,255,255,0.45)',
                        backdropFilter: active ? 'none' : 'blur(8px)',
                        WebkitBackdropFilter: active ? 'none' : 'blur(8px)',
                        boxShadow: active ? 'none' : '0 2px 8px rgba(0,0,0,0.1)',
