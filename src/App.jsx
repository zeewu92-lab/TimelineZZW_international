...
function LandmarkDetailModal({ ev, lang, t, isDark, onClose, onSetBgImage, onSetBgOpacity, onSetNumberFont, dock = false, closing = false }) {
  ...
  .overlay-opacity-range {
    ...
    background: linear-gradient(
      ...
      rgba(127,127,127,.18) var(--range-progress),
      rgba(127,127,127,.18) 100%
    );
    border-radius: 999px;
    outline: none;
    cursor: pointer;
  ...
  .overlay-opacity-range::-webkit-slider-runnable-track {
    height: 3px;
    background: transparent;
    border-radius: 999px;
  }

  .overlay-opacity-range::-webkit-slider-thumb {
    appearance: none;
    -webkit-appearance: none;
    width: 14px;
    height: 14px;
  ...
  .overlay-opacity-range::-moz-range-track {
    height: 3px;
    background: rgba(127,127,127,.18);
    border-radius: 999px;
  }

  .overlay-opacity-range::-moz-range-progress {
    height: 3px;
    background: var(--range-accent);
    border-radius: 999px;
  }

  .overlay-opacity-range::-moz-range-thumb {
    width: 14px;
    height: 14px;
    border: 1px solid rgba(255,255,255,.85);
    border-radius: 50%;
  ...
  .overlay-opacity-range:focus-visible {
    outline: 2px solid color-mix(in srgb, var(--range-accent) 34%, transparent);
    outline-offset: 3px;
  }
`}</style>
...
<input
                    ...
                    aria-label="背景遮罩透明度"
                    className="w-full overlay-opacity-range"
                    style={{
                      "--range-progress": `${Math.round(bgOpacity * 100)}%`,
                      "--range-accent": ACCENT,
                    }}
                  />
                </div>
...
function TimelineSection({
  ...
  // Ctrl+C（Mac 上對應 Cmd+C）快速呼出「新增地標」表單：
  // 只在「目前沒有選取文字、也沒有把焦點放在輸入框／可編輯區塊」時才攔截，
  // 避免蓋掉使用者原本想做的真的複製操作；表單已經開著（不管是新增還是編輯中）就不重複處理，
  // 也不會誤觸把正在編輯的內容關掉或重置。
  useEffect(() => {
